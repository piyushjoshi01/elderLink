...
const User = () => {
    ...
    const userData = () =>{
      ...
      if(!user){
        return(
            <div>User with this id dones't exist!</div>
        )
      }
    ...
    <div className="bg-gray-100 p-10 rounded-md shadow-md max-w-md">
        ...
        <div className="flex flex-col gap-3 text-lime-800 ">
            ...
            <p><strong>Address:</strong>
              <span>{`${user?.address?.street_name}, ${user?.address?.suite_number}, ${user?.address?.city}, ${user?.address?.state}, ${user?.address?.country}, ${user?.address?.pincode}`}</span></p>
        </div>
    </div>
</div>
<Footer/>
